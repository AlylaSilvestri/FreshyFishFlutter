...
class IkanTawarPageState extends State<IkanTawarPage> {
  ...
  late Future<List<dynamic>> freshFishProducts;
  ...
  void initState() {
    super.initState();
    me = getMe();
    freshFishProducts = getFreshFishProducts();
  }
  ...
  Future<List<dynamic>> getFreshFishProducts() async {
    String? token = await storageService.getToken();
    var response = await http.get(
      Uri.parse("https://freshyfishapi.ydns.eu/api/produk/habitat/Ikan Tawar"),
      headers: <String, String>{
        'Content-Type': 'application/json',
        'Authorization': "Bearer $token",
      },
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to load fresh fish products');
    }
  }
                  ...
                  FutureBuilder<List<dynamic>>(
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        ...
                        return const SliverToBoxAdapter(
                          child: Center(child: Text('No fresh fish available')),
                        );
                      }
